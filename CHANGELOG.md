...
- **Claude Code プラグインフォーマット修正**
  - plugin.json から非標準の commands/agents/skills 配列を削除
  - Claude Code の標準ディレクトリ自動検出機能を使用
  - 不要な marketplace.json を削除（単一プラグイン配布には不要）
  - プラグインインストールエラーを解消
...
- **JIRAチケットタイトルプレフィックス**: リポジトリ名と機能名を自動付与 (#128)
  - Epic: `[repo][feature] Project Name`
  - Story: `[repo][feature] Story: Title`
  - マルチプロジェクト環境での識別性向上
  - 新関数: `extractRepoName()`, `createTitlePrefix()`
