...
**生成されるファイル**:
- `.michi/settings/` - cc-sdd標準のルール、テンプレート（`.gitignore`対象）
- `.michi/steering/` - プロジェクト固有のコンテキスト
- `.michi/specs/` - 機能仕様（Git管理）
...
| コマンド | 機能説明 |
|---------|---------|
| `/michi:spec-init` | 新規仕様を初期化 |
| `/michi:spec-requirements` | 要件定義を作成 |
| `/michi:spec-design` | 設計書を作成 |
| `/michi:spec-tasks` | タスク分割とJIRA同期 |
| `/michi:spec-impl` | TDD実装を実行 |
