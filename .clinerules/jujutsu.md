...
> **NOTE: Single Source of Truth (SSOT)**
> 
> このファイル（`.cursor/rules/jujutsu.mdc`）がJujutsuルールの正規（canonical）ソースです。
> `.claude/jujutsu/jujutsu-rule.md`はこのファイルへの参照のみを含みます。
> 
> **更新方針**: 変更は常にこのファイルに対して行ってください。
...
### ❌ 禁止事項（Advisory - システムプロンプトまたはカスタムフックで強制推奨）

**⚠️ 重要**: Cursor v1.0.93以降、`.claude/settings.json`の`permissions.deny`設定はRead/Write/Toolツール操作に対して完全には機能しません。以下の制限は「推奨事項」として扱い、システムプロンプトまたはカスタムフックでの強制実装を検討してください。

**検証結果**: 
- 検証バージョン: Cursor v1.0.93+
- 観察された動作: denyルールが無視される事例が多数報告
- 参考: Stack Overflow、Redditでの開発者報告
...
3. **GitHub認証関連コマンドをAIエージェントが実行すること（Advisory）**
...
6. **Git pushやPR作成を実行すること（Advisory）**
...
**方法1: システムプロンプト組み込み**
```markdown
# .cursor/rules/ 内のファイルに以下を追加
- GitHub認証コマンド（gh auth *）の実行禁止
- Git pushコマンドの実行禁止  
- PR作成コマンド（gh pr create）の実行禁止
```
...
#!/bin/bash
if [[ "$1" =~ ^(gh\ auth|git\ push|gh\ pr\ create) ]]; then
    echo "❌ このコマンドの実行は制限されています"
    exit 1
fi
```

**アクションアイテム**: 
- [ ] Cursorのdeny設定不具合についてissue報告を作成
- [ ] 追跡用リンク: [要作成]
...
1. **終了マーカーの位置問題**: `EOF`は行の先頭に配置が必要
2. **インデント問題**: インデントされた`EOF`は認識されない
3. **複雑さの増加**: 不要なサブシェル実行が発生
