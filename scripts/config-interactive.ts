...
async function select(
  ...
  console.log(`⚠️  最大試行回数に達しました。デフォルト値を使用します: ${defaultValue || choices[0].value}`);
 ...
 * Confluence設定を対話的に取得
 */
async function getConfluenceConfig(rl: readline.Interface, projectMeta: any): Promise<any> {
  console.log('\n📄 Confluence設定');
  ...
  if (granularity === 'by-hierarchy' || granularity === 'manual') {
    ...
    if (granularity === 'by-hierarchy') {
      const modeChoices = [
        ...
        {
          value: 'nested',
          label: 'nested（親ページ → ドキュメントタイプ親 → セクション子ページ）',
          description: '3階層構造'
        }
      ];
    ...
    if (granularity === 'manual') {
      ...
      console.log('   hierarchy.structure に各ドキュメントタイプのページ構造を定義してください。');
    }
  }
...
async function getWorkflowConfig(rl: readline.Interface): Promise<any> {
  ...
  if (customApprovalGates) {
    ...
    if (enabledPhases.includes('requirements')) {
      ...
      if (approvers) {
        config.approvalGates.requirements = approvers.split(',').map(s => s.trim());
      }
    }
    ...
    if (releaseApprovers) {
      config.approvalGates.release = releaseApprovers.split(',').map(s => s.trim());
    }
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log(`📦 プロジェクト: ${projectMeta.projectName} (${projectMeta.projectId})\n`);
    } catch (error) {
    ...
    if (existsSync(configPath)) {
      ...
      try {
        ...
        console.log('既存の設定ファイルの読み込みに失敗しました。新規作成します。\n');
      }
    }
    ...
    // Confluence設定
    const configureConfluence = await confirm(rl, 'Confluence設定をカスタマイズしますか？', false);
    if (configureConfluence) {
      config.confluence = await getConfluenceConfig(rl, projectMeta);
    }
    
    // JIRA設定
    const configureJira = await confirm(rl, 'JIRA設定をカスタマイズしますか？', false);
    if (configureJira) {
      config.jira = await getJiraConfig(rl);
    }
    
    // ワークフロー設定
    const configureWorkflow = await confirm(rl, 'ワークフロー設定をカスタマイズしますか？', false);
