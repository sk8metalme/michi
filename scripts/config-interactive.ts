 ...
 * JIRA設定を対話的に取得
 */
async function getJiraConfig(rl: readline.Interface): Promise<JiraConfig> {
  console.log('\n📋 JIRA設定');
  console.log('='.repeat(60));
 ...
 * ワークフロー設定を対話的に取得
 */
async function getWorkflowConfig(rl: readline.Interface): Promise<WorkflowConfig> {
  console.log('\n⚙️  ワークフロー設定');
  console.log('='.repeat(60));
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 設定を対話的に取得
    const config: AppConfig = existingConfig || {};
