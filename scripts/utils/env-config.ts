...
const ENV_CONFIG: EnvValue[] = [
  ...
  },
  {
    ...
    description: 'Atlassian API Token (https://id.atlassian.com/manage-profile/security/api-tokens で生成)',
    required: true,
    sensitive: true
  },
  {
...
export function parseEnvFile(filePath: string): Map<string, string> {
  ...
  try {
    ...
    console.warn(`⚠️  Warning: Failed to parse ${filePath}:`, error instanceof Error ? error.message : error);
  }
 ...
 * @returns ユーザーの入力
 */
async function question(rl: readline.Interface, question: string): Promise<string> {
...
async function promptJiraIssueTypeId(
  ...
  if (hasJiraCredentials() && projectKey) {
    ...
    if (issueTypes && issueTypes.length > 0) {
      ...
      const filteredTypes = config.key === 'JIRA_ISSUE_TYPE_STORY'
      ...
      if (filteredTypes.length > 0) {
        ...
        if (!isNaN(index) && index >= 1 && index <= filteredTypes.length) {
          ...
          console.log(`   ✅ ${selected.name} (ID: ${selected.id}) を選択しました`);
          return selected.id;
        }
        ...
        if (foundById) {
          ...
          console.log(`   ✅ ${selected?.name} (ID: ${foundById}) を選択しました`);
          return foundById;
        }
        ...
        if (foundDirect) {
          console.log(`   ✅ ${foundDirect.name} (ID: ${foundDirect.id}) を選択しました`);
          return foundDirect.id;
        }
        
        // 見つからない場合、入力値をそのまま使用（ユーザーが手動でIDを入力した可能性）
        console.log(`   ⚠️  選択肢に一致しませんでしたが、入力値 "${input}" を使用します`);
        return input;
      } else {
        console.log(`   ⚠️  ${config.key === 'JIRA_ISSUE_TYPE_STORY' ? 'Story' : 'Subtask'}タイプが見つかりませんでした`);
      }
    } else {
      console.log(`   ⚠️  JIRA APIへのアクセスに失敗しました（認証情報が未設定、またはネットワークエラー）`);
      ...
      console.log(`   GET https://your-domain.atlassian.net/rest/api/3/project/${projectKey}`);
    }
  } else {
    if (!hasJiraCredentials()) {
      console.log(`   ℹ️  Atlassian認証情報が未設定のため、手動でIDを入力してください`);
    }
    if (!projectKey) {
      console.log(`   ℹ️  JIRAプロジェクトキーが未設定のため、手動でIDを入力してください`);
    }
  }
...
export async function promptEnvValue(
  ...
  if (config.key === 'JIRA_ISSUE_TYPE_STORY' || config.key === 'JIRA_ISSUE_TYPE_SUBTASK') {
    return promptJiraIssueTypeId(rl, config, projectKey, existingValue);
  }
  ...
  if (existingValue) {
    const displayValue = config.sensitive ? maskValue(existingValue) : existingValue;
...
export function generateEnvContent(values: Map<string, string>): string {
  ...
  // Atlassian設定
  const atlassianKeys = ['ATLASSIAN_URL', 'ATLASSIAN_EMAIL', 'ATLASSIAN_API_TOKEN'];
  ...
  // Confluence設定
  content += '\n# Confluence共有スペース\n';
  const confluenceKeys = ['CONFLUENCE_PRD_SPACE', 'CONFLUENCE_QA_SPACE', 'CONFLUENCE_RELEASE_SPACE'];
...
export async function configureEnvInteractive(
  ...
  try {
    ...
    console.log('各項目を設定してください。Enterキーのみで既存値またはデフォルト値を使用します。');
