...
Michiスキルは、Claude Codeの組み込みエージェントを活用して処理を並列化・効率化しています。

### エージェントタイプ別の活用

| エージェント | 特性 | Michiでの活用 |
|-------------|------|--------------|
| **Explore** | 読み取り専用、高速（Haikuモデル） | コードベース探索、パターン抽出、設計書分析 |
| **Bash** | コマンド実行専用 | npm/yarn実行、セキュリティスキャン、テスト |
| **general-purpose** | 全ツールアクセス、複雑な推論 | TDD実装、コードレビュー、ファイル編集 |
| **Plan** | 計画立案特化 | タスク分割、依存関係分析 |

### 対応スキル一覧

| スキル | 使用エージェント | 効果 |
|-------|----------------|------|
| analyze-gap | Explore x 3 | コードベース探索30%高速化 |
| review-design | Explore x 3 | レビュー品質向上 |
| review-dev | Explore x 2 + Bash x 3 | 検証タスク並列化 |
| multi-repo-review-cross | Explore x N | リポジトリ並列探索 |
| manage-todos scan | Explore x 4 | ドキュメント横断検索高速化 |
| dev (Phase 6.2, 6.5-6.6) | Bash x 3 | テスト・スキャン並行化 |
| create-tasks | Plan + Explore x 2 | タスク分割精度向上 |
| multi-repo-dev-all | Plan + general-purpose x N | 並行TDD実装 |

### 注意事項

- サブエージェントはスキルを継承しません
- 最大10個まで並列実行可能
- ネストしたサブエージェント呼び出しは禁止
- Bashエージェントはコマンド実行専用（ファイル編集不可）
- general-purposeエージェントはフル権限だがコスト注意
- Planエージェントは計画立案に特化、実装は不可

---

## まとめ

Michiスキルは、仕様駆動開発（SDD）からTDD実装まで、包括的な開発ワークフローをサポートします。フェーズ分離、品質ゲート、自動化、そしてサブエージェント活用により、高品質で効率的なソフトウェア開発を実現します。
