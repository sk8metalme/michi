...
設計書のレビューを3つのExploreエージェントで並列実行します：

### Phase 1: 並列チェック

| エージェント | 対象 | チェック項目 | 出力 |
|-------------|------|-------------|------|
| 技術的妥当性チェック | architecture.md, design.md | レイヤー分離、依存関係、正規化、RESTful | 技術的問題リスト |
| セキュリティチェック | design.md, sequence.md | OWASP Top 10、認証・認可、暗号化 | セキュリティ問題リスト |
| テスタビリティチェック | architecture.md, design.md | DI使用、インターフェース分離、モック容易性 | テスタビリティ問題リスト |
