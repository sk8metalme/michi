...
既存コードベースの探索を3つのExploreエージェントで並列実行します：

### Phase 1: 並列コードベース探索

| エージェント | 対象 | 目的 | 出力 |
|-------------|------|------|------|
| アーキテクチャ探索 | src/, lib/, packages/ | レイヤー構造、設計パターン特定 | アーキテクチャ構造レポート |
| API探索 | routes/, controllers/, api/ | 既存APIエンドポイント一覧 | API仕様リスト |
| データモデル探索 | models/, entities/, migrations/ | DBスキーマ、リレーション | データモデルリスト |

### Phase 2: 統合分析（メインエージェント）
